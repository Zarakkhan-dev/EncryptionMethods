...
const Algo = () => {
  ...
  const encryption = async () => {
    const response = await axios.post("http://127.0.0.1:8000/blowencryption/", {
      field,
    });
  ...
  const decryption = async () => {
    const response = await axios.post("http://127.0.0.1:8000/blowdecryption/", {
      encryptiontext,
      key,
            ...
            <section className="result flex gap-[20vh] justify-center mt-2">
              <div className="encryption-box">
                ...
                <div className="result-box w-[70vh] h-[20vh] bg-[#1f2937] text-white my-2 rounded-md p-3">
                  <p className=" overflow-hidden">{encryptiontext}</p>
                </div>
              </div>
